...
// This gets called by the recursive `emitAsmSubtree()` method to emit
// the pre-assembly for each node. The schema uses torch.aten.matmul
// which performs matrix multiplication.
inline std::string MatmulNode::emitNodePreAsm() const {
  // `torch.aten.matmul` signature from GeneratedTorchOps.td
  // https://github.com/llvm/torch-mlir/blob/main/include/torch-mlir/Dialect/Torch/IR/GeneratedTorchOps.td
  //
  //  def Torch_AtenMatmulOp : Torch_Op<"aten.matmul", [
  //    ...
  //    let summary = "Generated op for `aten::matmul : (Tensor, Tensor) ->
  //    (Tensor)`"; let arguments = (ins

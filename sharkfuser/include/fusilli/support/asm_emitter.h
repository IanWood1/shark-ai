...
inline std::string PointwiseNode::getOperandNamesAsm() const {
  ...
  case PointwiseAttr::Mode::RELU:
    return pointwiseAttr.getIN_0()->getValueNameAsm();
  case PointwiseAttr::Mode::ADD:
    return pointwiseAttr.getIN_0()->getValueNameAsm() + ", " +
           pointwiseAttr.getIN_1()->getValueNameAsm();
  default:
...
inline std::string PointwiseNode::getOperandTypesAsm() const {
  switch (pointwiseAttr.getMode()) {
  case PointwiseAttr::Mode::RELU:
    return pointwiseAttr.getIN_0()->getTensorTypeAsm(/*isValueTensor=*/true,
                                                     /*useLogicalDims=*/true);
  case PointwiseAttr::Mode::ADD:
    return pointwiseAttr.getIN_0()->getTensorTypeAsm(/*isValueTensor=*/true,
                                                     /*useLogicalDims=*/true) +
           ", " +
           pointwiseAttr.getIN_1()->getTensorTypeAsm(/*isValueTensor=*/true,
                                                     /*useLogicalDims=*/true);
  default:
    assert(false && "Unsupported pointwise mode");
    return "";

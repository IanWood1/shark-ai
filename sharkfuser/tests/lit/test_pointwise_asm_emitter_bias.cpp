...
// TORCH-CHECK:     func.func @main(%result_: !torch.tensor<[16,256,64,32],f32>, %arg0_input: !torch.vtensor<[16,256,64,32],f32>, %arg1_bias: !torch.vtensor<[1,256,1,1],f32>) attributes {torch.assume_strict_symbolic_shapes} {
// TORCH-CHECK:       %alpha_pointwise_bias = torch.constant.int 1
// TORCH-CHECK:       %result = torch.aten.add.Tensor %arg0_input, %arg1_bias, %alpha_pointwise_bias : !torch.vtensor<[16,256,64,32],f32>, !torch.vtensor<[1,256,1,1],f32>, !torch.int -> !torch.vtensor<[16,256,64,32],f32>
...
ErrorObject test_pointwise_asm_emitter_bias(const std::string &mode) {
  int64_t n = 16, c = 256, h = 64, w = 32;
  auto graph = std::make_shared<Graph>();
  graph->setName("pointwise_asm_emitter_bias");
  ...
  auto B = graph->tensor(TensorAttr()
                             .setName("arg1_bias")
                             .setDim({1, c, 1, 1})
                             .setStride({c, 1, 1, 1})); // 1D bias

  auto pointwise_attr = PointwiseAttr()
                            .setMode(PointwiseAttr::Mode::BIAS)
                            .setName("pointwise_bias");
...
int main(int argc, char **argv) {
  ...
  auto status = test_pointwise_asm_emitter_bias(mode);
  if (isError(status)) {

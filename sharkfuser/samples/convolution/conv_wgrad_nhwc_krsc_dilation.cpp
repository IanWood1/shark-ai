...
TEST_CASE(
    "Convolution wgrad; DY/X (NHWC), DW (KRSC); 3x3; dilation=2; no padding",
    "[conv][graph]") {
  int64_t n = 4, c = 8, h = 12, w = 12, hx = 16, wx = 16, k = 16, r = 3, s = 3;

  auto buildNewGraph = [=](const Handle &handle) {
    auto graph = std::make_shared<Graph>();
    graph->setName("conv_wgrad_sample_nhwc_krsc_3x3_dilation2_nopad");
    ...
    auto xT =
        graph->tensor(TensorAttr()
                          .setName("x")
                          .setDim({n, c, hx, wx})
                          .setStride({c * hx * wx, 1, c * wx, c})); // NHWC

    auto wgradAttr = ConvWGradAttr()
                         .setStride({1, 1})
                         .setPadding({0, 0})
                         .setDilation({2, 2})
                         .setName("conv_wgrad_dilation");
  ...
  // Use values of 1.0 so the resulting DW for 3x3 conv with dilation equals
  // N*H*W.
  ...
  // Read output buffer and validate values for 3x3 with dilation=2, stride=1,
  // no padding.
